...
def log_prior_g(theta_g):
    prior_beta = torch.distributions.normal.Normal(torch.tensor([0.]),torch.tensor([10.])).log_prob(theta_g[:-1]).sum()
    prior_sigma = torch.log(torch.tensor([2.0]))+torch.distributions.studentT.StudentT(df=2.0,scale=10.0).log_prob(theta_g[-1].exp())+theta_g[-1]
